    ...
    for(int i = 0; i < synth.getNumVoices(); ++i)
       {
           ...
           if((thisVoice =  dynamic_cast<FourierSynthVoice*>(synth.getVoice(i))))
           {
               ...
               {
                   scopeSource.addSampleToBlock(thisVoice->proc.lastVoiceSample);
               } 
               scopeSource.voiceFundamental = (thisVoice->proc.lastFundamental);
               scopeSource.processorSampleRate = getSampleRate();
           }
       }
